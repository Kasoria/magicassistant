...
class React_Dev {
    ...
    /**
     * Enqueue Bricks text enhancer React scripts
     */
    private function enqueue_bricks_text_enhancer_scripts( $hook ) {
        if ( $this->is_dev_mode ) {
            $this->enqueue_bricks_text_enhancer_dev_scripts( $hook );
        } else {
            $this->enqueue_bricks_text_enhancer_prod_scripts( $hook );
        }
        $this->localize_bricks_text_enhancer_data();
    }
    ...
    /**
     * Enqueue Bricks text enhancer development scripts from Vite dev server
     */
    private function enqueue_bricks_text_enhancer_dev_scripts( $hook ) {
        // Vite client for HMR - only enqueue if not already enqueued
        if ( ! wp_script_is( 'vite-client', 'enqueued' ) ) {
            wp_enqueue_script(
                'vite-client',
        ...
        // Bricks text enhancer React app from Vite dev server
        wp_enqueue_script(
    ...
    /**
     * Enqueue Bricks text enhancer production built scripts
     */
    private function enqueue_bricks_text_enhancer_prod_scripts( $hook ) {
        $dist_path = MAGIC_ASSISTANT_PLUGIN_PATH . 'dist/';
        $dist_url = MAGIC_ASSISTANT_PLUGIN_URL . 'dist/';
        ...
        // Bricks text enhancer React app
        if ( file_exists( $dist_path . 'bricks-text-enhancer.js' ) ) {
            wp_enqueue_script(
                'mat-react-bricks-text-enhancer',
                $dist_url . 'bricks-text-enhancer.js',
                $vendor_handles,
                MAGIC_ASSISTANT_VERSION,
                true
            );
    ...
    /**
     * Localize data for Bricks text enhancer React app
     */
